...
try:
    ...
    if recent_highlights:
        ...
        print(
            f'📅 Highlighted: {latest.highlighted_at.strftime("%Y-%m-%d %H:%M") if latest.highlighted_at else "Unknown date"}'
        )
        print(
            f'💡 Source: {latest.book.source if latest.book and latest.book.source else "Unknown"}'
        )
        print()
        print('"' + latest.text + '"')
